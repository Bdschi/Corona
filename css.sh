...
END {
	for(Province in cyp) {
		if(pyp[Province]>0 && plp[Province]>0) {
			...
			printf "%.2f,%s,%d,%d,%d,%d\n", (cyp[Province]-clp[Province])*100000.0/pop, Province, cyp[Province], pyp[Province], clp[Province], plp[Province];
		}
	}
	for(Country in cy) {
		if(py[Country]>0 && pl[Country]>0) {
			pop=(py[Country]+pl[Country])/2
			printf "%.2f,%s,%d,%d,%d,%d\n", (cy[Country]-cl[Country])*100000.0/pop, Country, cy[Country], py[Country], cl[Country], pl[Country];
		}
	}
}' <( ./csv2tsv.py < $file) <( ./csv2tsv.py < $filew) | sort -n | gzip > cssout.`date +"%Y%m%d"`.txt.gz
zcat cssout.`date +"%Y%m%d"`.txt.gz | egrep 'France,|Germany|Croatia|Netherlands,|Belgium|Slovenia|United Kingdom,|US,|Spain|Italy|Monaco'
